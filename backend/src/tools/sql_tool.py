...
def query_financial_database(sql_query: str) -> str:
    ...
    try:
        ...
        rows = "\n".join(
            [" | ".join(str(v) for v in row) for row in results]
        )
...
def get_revenue_breakdown(company: str, year: int) -> str:
    """
    Get revenue breakdown by segment and geography for a company.
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT segment, revenue, growth_pct
